public class ListBorrowers {
  ...
  /**
   * Ajoute un nouvel adhérent dans la liste. Cette méthode vérifie si l'adhérent est déjà présent.
   *
   * @param theBorrower le nouvel adhérent ajouté
   * @return vrai si l'ajout dans la table est réussi, faux si un adhérent avec une même clé existe
   *     déjà dans la table
   */
  public boolean addNewBorrower(Borrower theB) {
    ...
    if (this.theBorrowers.contains(theB)) {
      ret = false;
    } else {
      this.theBorrowers.add(theB);
    }
  ...
  /**
   * Méthode qui transforme TOUT le tableau en une chaîne de caractères affichable ou imprimable
   * dans un fichier
   */
  ...
  /**
   * Ecrit l'ensemble des adhérents dans le fichier texte. Une ligne du fichier = tous les champs
   * d'un adhérent séparés par le caractère "&".
   *
