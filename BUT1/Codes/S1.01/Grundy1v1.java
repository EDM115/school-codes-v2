/**
 * Jeu de Grundy - Joueur contre joueur
 *
 * @author EDM115
 */
...
class Grundy1v1 {
   ...
   */
  String nomJoueur(int nb) {
    String joueur = SimpleInput.getString("Nom du joueur " + nb + " : ");
    while (joueur.length() < 1) { // Test probablement inutile
      joueur = SimpleInput.getString("Nom du joueur " + nb + " : ");
    }
    return joueur;
  }
      ...
      while (divAllumettes < 1
          ...
          || divAllumettes
              == Math.ceil(allumettesDansLigne(reponses[divLigne]))
                  / 2) { // On vérifie que le nombre d'allumettes ne dépasse pas celui disponible et
        // qu'on peut faire 2 tas distincts
