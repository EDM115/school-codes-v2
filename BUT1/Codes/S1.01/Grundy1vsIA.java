...
class Grundy1vsIA {
  ...
  /**
   * Demande au joueur le nombre d'allumettes pour la partie, ça doit être supérieur ou égal à 3
   * (sinon bah y'a pas de jeu)
   *
  ...
  void testCasGuessAllumettes(int lg, String[] rep, int result) {
    // Arrange
    System.out.print(
        "testCasGuessAllumettes(" + Arrays.toString(rep) + ", " + lg + ") \t= " + result + "\t : ");
    // Act
  ...
  int[] guessMeilleurCoup(String[] reponses) {
    ...
    choix[0] = 0;
    if (reponses.length
        > 1) { // Pour ne pas demander inutilement la ligne si on est au premier coup
  ...
  void testDivise() {
    ...
    String[] result1 = {
      creerReponse(2), creerReponse(1), creerReponse(2), creerReponse(3), creerReponse(4)
    };
    String[] result2 = {
      creerReponse(2), creerReponse(1), creerReponse(1), creerReponse(4), creerReponse(4)
    };
    String[] result3 = {
      creerReponse(2), creerReponse(1), creerReponse(5), creerReponse(3), creerReponse(1)
    };
  ...
  int[] verifPartieFinie(String[] reponses, int nbAllumettes) {
    ...
    int i = 0;
    while (uneSeuleLigneUtilisable
        && j < len) { // On vérifie qu'il n'y a qu'une seule ligne avec + de 2 allumettes
      if (lignes[j] > 2) {
        i++;
      }
      if (i > 1) {
    ...
    while (!partieFinie) {
      ...
      if (joueur == j1) {
        ...
        if (reponses.length
          ...
          divLigne = numLigne();
          while (divLigne < 0
              || divLigne >= reponses.length
              || (divLigne <= reponses.length
                  && allumettesDansLigne(reponses[divLigne])
                      < 3)) { // On vérifie que la ligne est utilisable, et qu'il y a plus de 2
            // allumettes dedans
            divLigne = numLigne();
          }
        }
        int divAllumettes = numAllumettes();
        while (divAllumettes < 1
            || divAllumettes >= allumettesDansLigne(reponses[divLigne])
            || divAllumettes
                == Math.ceil(allumettesDansLigne(reponses[divLigne]))
                    / 2) { // On vérifie que le nombre d'allumettes ne dépasse pas celui disponible
          // et qu'on peut faire 2 tas distincts
        ...
        if (allumettes % 2 != 0
            && nbCoupsIA
                == 0) { // Si le nombre d'allumettes est impair et qu'on est au premier coup, on
          // enlève 2 allumettes au lieu d'une seule
          choix[1] = 2;
        }
        ...
        if (bestChoice[0]
            == -1) { // Si on a pas déterminé de coup gagnant, on prend le coup de l'IA
