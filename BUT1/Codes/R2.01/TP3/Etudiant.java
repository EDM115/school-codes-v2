...
public class Etudiant {
	...
	public Etudiant(String nom, String[] matieres, double[] coeff, int nbNotes) {
		if (nom == null) {
			...
			System.err.println("Etudiant : Le tableau des matières et le tableau des coefficients doivent avoir la même taille");
			this.MATIERES = new String[0];
			this.COEFFICIENTS = new double[0];
		} else if (nbNotes < 0) {
	...
	public double getUneNote(int iMatiere, int i) {
		...
		if (iMatiere < 0 || iMatiere >= this.bulletin.length) {
			System.err.println("getUneNote : L'indice de la matière doit être compris entre 0 et " + (this.bulletin.length - 1));
		} else if (i < 0 || i >= this.bulletin[iMatiere].length) {
			System.err.println("getUneNote : L'indice de la note doit être compris entre 0 et " + (this.bulletin[iMatiere].length - 1));
		} else {
	...
	public String toString() {
		...
		if (this.nom == null || this.bulletin == null || this.MATIERES == null || this.COEFFICIENTS == null) {
			System.err.println("toString : Un des attributs est null");
		} else {
			str += "Nom : " + this.nom + "\nMatières : " + Arrays.toString(this.MATIERES) + "\nCoefficients : " + Arrays.toString(this.COEFFICIENTS) + "\nBulletin : " + this.betterBulletin(this.bulletin);
		}
	...
	/**
	 * From a given Etudiant object, returns informations about it (the complete bulletin as expected to be)
