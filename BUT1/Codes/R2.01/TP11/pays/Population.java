...
public class Population {
	 ...
	 */
	public Pays[] getListePays() {
		return listePays.toArray(new Pays[listePays.size()]);
	}
	...
	/**
	 * Retourne la densité de chaque pays
	 * @return La densité de chaque pays dans une HashMap
	 */
	public HashMap<String, Double> computeDensity() {
		HashMap<String, Double> densityMap = new HashMap<String, Double>();

		for (Pays pays : listePays) {
			densityMap.put(pays.getNom(), pays.getPopulation() / pays.getSurface());
		}
	...
	*/
	public static void writeMap(HashMap<String, Double> map, String fileName) {
		if (map == null || fileName == null || fileName.length() == 0) {
			System.err.println("\u001B[31mwriteMap() : Invalid arguments\u001B[0m");
		} else {
			try {
