...
/**
 * Classe TriParSelectionAlpha, implémentant l'interface ITri
 */
public class TriParSelectionAlpha implements ITri<Pays> {
    ...
    private int minimumPosition(int debut) {
        if (debut < 0 || debut >= this.tab.length) {
            ...
            for (int i = debut + 1; i < this.tab.length; i++) {
                if (this.tab[i].getNom().compareToIgnoreCase(this.tab[minPos].getNom()) < 0) {
                    minPos = i;
                }
            }
            return minPos;
    ...
    /**
     * Trie le tableau tab par ordre alphabétique des noms de pays
     *
    ...
    /**
     * Constructeur de la classe TriParSelectionAlpha
     *
     * @param tab le tableau à trier
     */
    public TriParSelectionAlpha(Pays[] tab) {
        if (tab == null) {
            System.err.println("\u001B[31mTriParSelectionAlpha() : Invalid arguments\u001B[0m");
        } else {
            this.trier(tab);
        }
    }
    ...
    /**
     * Retourne le tableau de pays
     * @return le tableau de pays
     */
    public Pays[] getTab() {
        return tab;
    }
