...
/**
 * Cette classe effectue des opérations élémentaires sur un ou plusieurs tableaux d'entiers. La
 * taille d'un tableau est par définition le nombre TOTAL de cases = tab.length. Un tableau
 * d'entiers créé possède nb éléments qui est nécessairement inférieur ou égal à la taille du
 * tableau : nb &lt;= tab.length (= taille)
 *
 * @author EDM115
 */
public class SimplesTableau {
  ...
  /**
   * Factorisation : appelé chaque fois qu'une vérification doit se faire sur la validité d'un
   * tableau. Le tableau n'est pas valide si celui-ci est inexistant ou si le nombre d'éléments
   * qu'il contiendra est incompatible avec sa taille (tab.length)
   *
   * @param tab le tableau dont on doit vérifier l'existence (tab différent de null)
   * @param nb le nombre d'éléments que contiendra le tableau, doit vérifier la condition 0 &lt; nb
   *     &lt;= tab.length
  ...
  /**
   * Affiche le contenu des nb cases d'un tableau une par une. Tenir compte des cas d'erreurs
   * concernant tab et nb (appel de verifTab)
   *
  ...
  /**
   * Renvoie un entier aléatoire compris entre min et max (min &lt;= valeur &lt;= max). Vérifier que
   * min &lt;= max et min &gt;= 0, sinon afficher une erreur (et dans ce cas retourner -1)
   *
  ...
  /**
   * A partir d'un tableau créé, remplir aléatoirement ce tableau de nb valeurs comprises entre min
   * et max. Tenir compte des cas d'erreurs concernant tab et nb (appel de verifTab). Vérifier que
   * min &lt;= max, sinon afficher également une erreur. Utiliser obligatoirement la méthode "int
   * tirerAleatoire(int min, int max)"
   *
   ...
   * d'éléments et en contenu (case par case). Tenir compte des cas d'erreurs concernant tab1, nb1
   * et tab2, nb2 (appel de verifTab 2 fois) et afficher un éventuel message d'erreur (et dans ce
   * cas renvoyer faux)
   *
  ...
  /**
   * Renvoie un nouveau tableau qui est l'inverse de celui passé en paramètre. Son j-ème élément est
   * égal au (nb + 1 - j) élément du tableau initial (dans l'explication, j = 1 signifie premier
   * élément du tableau)
   *
   * @param tab le tableau
   * @param nb le nombre d'entiers présents dans le tableau
  ...
  /**
   * Echange les contenus des cases du tableau passé en paramètre, cases identifiées par les indices
   * ind1 et ind2. Vérifier que les indices ind1 et ind2 sont bien compris entre zéro et (nb - 1),
  ...
  void testCasEchange(int[] tab, int nb, int ind1, int ind2, int[] result, boolean err) {
    System.out.print(
        "testCasEchange(" + Arrays.toString(tab) + ", " + nb + ", " + ind1 + ", " + ind2 + ") : ");
  ...
  /**
   * Retourne un nouveau tableau qui a la même taille et les mêmes occurrences d'élements que le
   * tableau passé en paramètre mais ces éléments sont répartis selon des indices aléatoires (0
   * &lt;= indice &lt;= nbElem - 1). Une technique simple consiste à utiliser les méthodes "echange"
   * et "tirerAleatoire" pour effectuer le mélange
   *
   * @param tab le tableau
   * @param nb le nombre d'entiers présents dans le tableau
  ...
  void testMelange() {
    ...
    // Cas d'erreur
    // int[] tab2 = {8};
    // testCasMelange(tab2, 1, true); (c'est tellement un cas d'erreur que Java aime pas exécuter
    // ça)
  ...
  /**
   * Décale de une case de la droite vers la gauche toutes les cases d'un tableau à partir d'un
   * indice "ind" et jusque nb - 1 ([ind] &lt;- [ind + 1] &lt;- [ind + 2] &lt;- ... &lt;- [nb - 2]
   * &lt;- [nb - 1]). Vérifier que ind est compris entre 0 et (nb - 2) sinon afficher une erreur
   *
   * @param tab le tableau
   * @param nb le nombre d'entiers présents dans le tableau
  ...
  void testCasDecalerGauche(int[] tab, int nb, int ind, int[] result, boolean err) {
    System.out.print(
        "testCasDecalerGauche(" + Arrays.toString(tab) + ", " + nb + ", " + ind + ") : ");
  ...
  /**
   * Supprime du tableau la première case rencontrée dont le contenu est égale à "valeur". La case
   * du tableau est supprimée par décalage à gauche des cases du tableau. L'appel de la méthode
   * "decalerGauche" est obligatoire. A l'issue de la suppression (si elle existe) le nombre
   * d'éléments dans le tableau est décrémenté et retourné
   *
   * @param tab le tableau
   * @param nb le nombre d'entiers présents dans le tableau
  ...
  void testCasSupprimerUneValeur(int[] tab, int nb, int valeur, int result, boolean err) {
    System.out.print(
        "testCasSupprimerUneValeur(" + Arrays.toString(tab) + ", " + nb + ", " + valeur + ") : ");
  ...
  /**
   * Renvoie vrai ssi le tableau tab1 est inclus dans tab2. Autrement dit, si tous les éléments de
   * tab1 se retrouvent intégralement dans tab2 (y compris les doublons) mais pas nécessairement
   * dans le même ordre. L'utilisation de méthodes déjà écrites est autorisé
   *
  ...
  /**
   * Une méthode utilitaire pour remplirToutesDiff. Vérifie si une valeur existe dans un tableau
   * sauf à un certain indice (où on compare avec preval)
   *
  ...
  /**
   * A partir d'un tableau déjà créé, remplir le tableau de nb valeurs saisies par l'utilisateur. Au
   * fur et à mesure de la saisie, si la nouvelle valeur saisie existe déjà dans le tableau alors ne
   * pas l'insérer et demander de ressaisir. Tenir compte des cas d'erreurs concernant tab et nb
   * (appel de verifTab)
   *
    ...
    if (!verif) {
      ...
      while (i < nb) {
        ...
        while (estPresent(tab, tab[i], i, preval)) {
          tab[i] =
              SimpleInput.getInt(
                  "Entrez une valeur à insérer (elle ne doit pas déjà être dans le tableau) : ");
          System.out.println(Arrays.toString(tab));
        }
        i++;
      }
