...
class Pendu {
	 ...
	 **/
	String[] creerDico() {
		String[] dico = {"administrateur", "android", "apple", "binaire", "bluetooth", "code", "discord", "emoji", "gamer", "google", "java", "informatique", "internet", "linux", "mail", "microsoft", "programmation", "python", "rgb", "serveur", "smartphone", "usb", "vpn", "wifi", "windows", "wooclap"};
		return dico;
	}
	...
	void testCasTester(String mot, char[] reponse, char car, boolean resultat) {
		...
		// Arrange
		System.out.print("testCasTester(" + mot + ", " + affStringChar(reponse) + ", " + car + ") \t= " + resultat + "\t : ");
		// Assert
		if (tester == resultat){
			System.out.println("OK");
		} else {
	...
	void testCasEstComplet(String mot, char[] reponse, boolean resultat) {
		// Arrange
		System.out.print("testCasEstComplet(" + mot + ", " + affStringChar(reponse) + ") \t= " + resultat + "\t : ");
		// Act
	...
	void partie(String[] dico) {
		...
		if (err < 9) {
			...
			System.out.println("Vous n'avez pas trouvé " + mot + ". Vous avez fait 9 erreurs en " + i + " essais");
		}
	}
