...
void reader(const int num, std::shared_timed_mutex& rw_mutex) {
...
void writer(const int num, std::shared_timed_mutex& rw_mutex) {
...
int main()
{
    ...
    for (auto& t : threads_reader) {
        t = std::jthread(reader, num, std::ref(rw_mutex));
        ++num;
    }
    std::vector<std::jthread> threads_writer(nb_writers);
    num = 0;
    for (auto& t : threads_writer) {
        t = std::jthread(writer, num, std::ref(rw_mutex));
        ++num;
    }
