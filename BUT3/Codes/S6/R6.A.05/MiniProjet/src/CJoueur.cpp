...
void CJoueur::attaque(pair<int, int> tir, CCoups& e) {
  ...
  cout << endl
       << "Attaque sur l'adversaire : coord. ligne = " << tir.first << "\tcoord. colonne = " << tir.second << endl;
  ...
  *m_pSocCli >> str;
  ...
  cout << endl
       << "Reponse de l'adversaire suite a l'attaque : " << str << endl;

  // mise à jour de la structure de données qui comptabilise les tirs (dansLEau ou touche ou coule) du joueur
  // dansLEau
  if (str == "dansLEau") {
    e.ajouterTirMap("dansLEau", tir);
  }
  // touché ou coulé
  else {
    e.ajouterTirMap("touche", tir);
  }
}
...
void CJoueur::attaqueAdverse(CArmada* f, CCoups* e) {
  ...
  // il y a 2 coordonnées : n1=tir.first et n2=tir.second qui sont r�ceptionn�es
